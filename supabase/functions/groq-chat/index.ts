...
const PROVIDERS: ProviderDef[] = [
  ...
  },
  {
    ...
    buildRequest: (apiKey, msgs, maxTokens) => ({
      ...
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
      body: { model: "grok-2-1212", messages: msgs, max_tokens: maxTokens },
    }),
    extractReply: (data) => data?.choices?.[0]?.message?.content ?? null,
  },
  {
    ...
    buildRequest: (apiKey, msgs, maxTokens) => ({
      ...
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
      body: { model: "google/gemini-2.0-flash-001", messages: msgs, max_tokens: maxTokens },
    }),
    extractReply: (data) => data?.choices?.[0]?.message?.content ?? null,
  },
  {
    ...
    buildRequest: (apiKey, msgs, maxTokens) => ({
      ...
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
      body: { model: "llama3.1-8b", messages: msgs, max_tokens: maxTokens },
    }),
    extractReply: (data) => data?.choices?.[0]?.message?.content ?? null,
  },
  {
